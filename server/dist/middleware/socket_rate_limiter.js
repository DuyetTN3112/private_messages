...
exports.cleanup_socket_store = exports.should_rate_limit_message = exports.socket_rate_limiter = void 0;
 ...
 */
const socket_rate_limiter = (socket, next) => {
    // Tạo entry mới cho mỗi kết nối
    socket_store[socket.id] = {
        message_count: 0,
        last_reset: Date.now(),
        message_timestamps: []
    };
...
exports.socket_rate_limiter = socket_rate_limiter;
/**
 * Kiểm tra và áp dụng rate limit cho mỗi tin nhắn
 * @returns true nếu message nên được chặn
 */
const should_rate_limit_message = (socket_id) => {
    const now = Date.now();
    ...
    if (client_data.blocked_until && now < client_data.blocked_until) {
        ...
        logger_1.logger.warn(`Socket rate limit: Client ${socket_id} is blocked. Remaining time: ${remaining_seconds}s`);
        return true;
    }
    ...
    if (now - last_message_time < MIN_MESSAGE_INTERVAL) {
        ...
        logger_1.logger.warn(`Socket rate limit: Client ${socket_id} is sending messages too fast. Blocked for ${BLOCK_DURATION / 1000}s`);
        return true;
    }
    // Thêm timestamp của tin nhắn mới
    client_data.message_timestamps.push(now);
    // Giữ timestamps trong cửa sổ hiện tại
    client_data.message_timestamps = client_data.message_timestamps.filter(time => now - time < RATE_LIMIT_WINDOW);
    ...
    if (client_data.message_count > MAX_MESSAGES_PER_WINDOW) {
        client_data.blocked_until = now + BLOCK_DURATION;
        logger_1.logger.warn(`Socket rate limit: Client ${socket_id} exceeded message limit. Blocked for ${BLOCK_DURATION / 1000}s`);
        return true;
    }
    // Cho phép gửi tin nhắn
    return false;
};
exports.should_rate_limit_message = should_rate_limit_message;
/**
 * Làm sạch store khi socket ngắt kết nối
 */
const cleanup_socket_store = (socket_id) => {
    if (socket_store[socket_id]) {
        delete socket_store[socket_id];
    }
};
exports.cleanup_socket_store = cleanup_socket_store;
/**
