...
exports.sanitize_message = exports.is_valid_message = exports.validate_message = void 0;
 ...
 */
const has_excessive_repeats = (text, max_repeats = 5) => {
 ...
 * @param max_repeats Số lần lặp lại tối đa cho phép
 */
const has_repeated_words = (text, max_repeats = 3) => {
 ...
 */
const restore_urls = (processed_message, urls) => {
...
const validate_message = (message) => {
    ...
    if (!message || typeof message !== 'string') {
        throw new error_handler_1.ApiError('Nội dung tin nhắn không hợp lệ', 400);
    }
    // Kiểm tra độ dài tin nhắn
    if (message.length > MAX_MESSAGE_LENGTH) {
        throw new error_handler_1.ApiError(`Tin nhắn không được vượt quá ${MAX_MESSAGE_LENGTH} ký tự`, 400);
    }
    // Kiểm tra tin nhắn rỗng hoặc chỉ có khoảng trắng
    if (message.trim().length === 0) {
        throw new error_handler_1.ApiError('Tin nhắn không được để trống', 400);
    }
    ...
    if (has_excessive_repeats(processed_message)) {
        throw new error_handler_1.ApiError('Tin nhắn chứa quá nhiều ký tự lặp lại', 400);
    }
    // Kiểm tra từ lặp lại quá nhiều
    if (has_repeated_words(processed_message)) {
        throw new error_handler_1.ApiError('Tin nhắn chứa quá nhiều từ lặp lại liên tiếp', 400);
    }
    // Kiểm tra regex dấu nhiều
    if (EXCESSIVE_DIACRITICS_REGEX.test(processed_message)) {
        throw new error_handler_1.ApiError('Tin nhắn chứa quá nhiều dấu', 400);
    }
    // Kiểm tra ký tự hợp lệ - chỉ áp dụng cho phần không phải URL
    if (!VALID_CHARS_REGEX.test(processed_message)) {
        throw new error_handler_1.ApiError('Tin nhắn chứa ký tự không hợp lệ', 400);
    }
};
...
const is_valid_message = (message) => {
    try {
        (0, exports.validate_message)(message);
        return true;
    }
    catch (error) {
        return false;
...
exports.sanitize_message = sanitize_message;
