...
export const should_rate_limit_message = (socket_id: string): boolean => {
  ...
  const client_data = socket_store.get(socket_id);
  ...
  if (client_data.blocked_until !== undefined && now < client_data.blocked_until) {
    ...
    logger.warn(`Socket rate limit: Client ${socket_id} is blocked. Remaining time: ${String(remaining_seconds)}s`);
    return true;
  }
  ...
  const last_message_time = client_data.message_timestamps.at(-1) ?? 0;
  ...
  if (now - last_message_time < MIN_MESSAGE_INTERVAL) {
    ...
    logger.warn(`Socket rate limit: Client ${socket_id} is sending messages too fast. Blocked for ${String(BLOCK_DURATION/1000)}s`);
    return true;
  }
  ...
  if (client_data.message_count > MAX_MESSAGES_PER_WINDOW) {
    client_data.blocked_until = now + BLOCK_DURATION;
    logger.warn(`Socket rate limit: Client ${socket_id} exceeded message limit. Blocked for ${String(BLOCK_DURATION/1000)}s`);
    return true;
  }
...
setInterval(() => {
  const now = Date.now();
  for (const [socket_id, client_data] of socket_store.entries()) {
    ...
    if (now - client_data.last_reset > CLEANUP_INTERVAL && !is_blocked) {
      socket_store.delete(socket_id);
    }
  }
}, CLEANUP_INTERVAL); 